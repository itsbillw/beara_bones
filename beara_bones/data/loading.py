    ...
    for _, row in df.iterrows():
        ...
        records.append(
            Fixture(
                ...
                if pd.notna(row.get("fixture_id"))
                else None,
                date=date_val,
                ...
                if pd.notna(row.get("venue_id"))
                else None,
                venue_name=str(row.get("venue_name", ""))
                if pd.notna(row.get("venue_name"))
                else "",
                status_short=str(row.get("status_short", ""))
                if pd.notna(row.get("status_short"))
                else "",
                status_long=str(row.get("status_long", ""))
                if pd.notna(row.get("status_long"))
                else "",
                ...
                league_season=int(row["league_season"])
                if pd.notna(row.get("league_season"))
                else season,
                league_round=str(row.get("league_round", ""))
                if pd.notna(row.get("league_round"))
                else "",
                ...
                if pd.notna(row.get("goals_home"))
                else None,
                goals_away=int(row["goals_away"])
                if pd.notna(row.get("goals_away"))
                else None,
            ),
        )
    Fixture.objects.bulk_create(records)
    logger.info(
        "Loaded %d fixtures for league_id=%s season=%s",
        len(records),
        league_id,
        season,
    )
